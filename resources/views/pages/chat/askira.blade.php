...
<div class="container">
  ...
  @foreach ($askirachats as $chat)
    ...
    @else
      <div class="d-flex align-items-end flex-column">
        <p class="fw-bold fs-5">{{ $chat->user->fullname }}</p>
        <p class="small p-1 text-white rounded-3 bg-primary">{{ htmlspecialchars($chat->message) }}</p>
  ...
  <form method="POST" action="{{ route('send.chat') }}" class="text-muted d-flex justify-content-start align-items-center p-1" id="sendChat">
    ...
    {{-- <input type="text" class="form-control form-control-lg" id="editor" placeholder="Type message" name="message"> --}}
